...
{
  detail::forkpool::get();  // Make sure static-variables are initialized before
                            // constructing task
...
public:
  ...
  private:
    struct final_awaitable : std::suspend_always
    {
          ...
          std::coroutine_handle<promise_type> task) const noexcept
      {
        ...
        std::uint64_t n =
            (task.promise()._parent_n)->fetch_sub(1, std::memory_order_acq_rel);
  ...
  public:
    ...
    auto await_transform(detail::tag_fork<U> child) const noexcept
    {
      ...
      struct awaitable : std::suspend_always
      {
            ...
            std::coroutine_handle<promise_type> parent)
        {
          ...
          detail::forkpool::push(
              {parent, std::addressof(parent.promise()._alpha)});
    ...
    auto await_transform(tag_sync) noexcept
    {
      struct awaitable
      {
        ...
        std::coroutine_handle<> await_suspend(
            std::coroutine_handle<promise_type> task) noexcept
        {
          ...
          std::uint64_t n =
              task.promise()._n.fetch_sub(IMAX - a, std::memory_order_acq_rel);
  ...
  private:
    ...
    alignas(hardware_destructive_interference_size) std::atomic_uint64_t _n =
        IMAX;

    static constexpr std::uint64_t IMAX =
    ...
    template<typename U>
    void set_parent(
        std::coroutine_handle<typename Task<U>::promise_type> parent) noexcept
    {
      _parent = parent;
  ...
  Task& operator=(Task&& other) noexcept
  {
    if (this != &other) {
      destroy(
          std::exchange(_coroutine, std::exchange(other._coroutine, nullptr)));
    }
    return *this;
  }
...
private:
  ...
  auto fork() &&
  {
    return detail::tag_fork<T> {std::exchange(_coroutine, nullptr)};
  }
  ...
  decltype(auto) root() &&
  {
    if (_coroutine) {
      ...
      detail::forkpool::push_root(
          {_coroutine, std::addressof(_coroutine.promise()._alpha)});
