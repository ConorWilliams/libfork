...
inline thread_local std::size_t root_id =
...
public:
  ...
  // Can only throw on first call hence tag above as noexcept as only main
  // thread can throw
  static forkpool& get()
  {
...
private:
  alignas(hardware_destructive_interference_size)
      std::atomic<std::int64_t> _actives = 0;
  alignas(hardware_destructive_interference_size)
      std::atomic<std::int64_t> _thieves = 0;
  alignas(hardware_destructive_interference_size) std::atomic<bool> _stop =
      false;
