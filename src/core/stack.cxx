...
class geometric_stack {
 public:
  constexpr auto checkpoint() noexcept -> root * { return m_root.get(); }

  constexpr auto acquire(root *ckpt) noexcept -> void {
    if (ckpt != m_root.get()) {
      // If not part of
      // Then we should be empty
      LF_ASSUME(!m_root)
 ...
 private:
  struct buffer {
    std::uniqe_ptr<std::byte[]> data;
    std::size_t size;

    constexpr buffer(std::size_t size) : data(std::make_unique_for_overwrite<std::byte[]>(size)), size(size) {
      LF_ASSUME(std::bit_cast<std::uintptr_t>(data.get()) % k_new_align == 0);
    }
  };

  // Just a buffer + stack pointer cache.
  struct stacklet {
    buffer stack;                  // The actuall stack buffer.
    stacklet *prev;                // Linked list of stacklets.
    std::byte *sp_cache = nullptr; // Cache of the current stack pointer.
  }

  // Heap allocated stack control structure
  struct stack_control {
    std::stack<void *> debug;        // Store data for debugging purposes.
    std::unique_ptr<stacklet> top;   // The top stacklet.
    std::unique_ptr<stacklet> cache; // A cached stacklet for the next allocation.
  };

  std::unique_ptr<stack_control> m_ctrl = nullptr; // Owns the root
  std::byte *m_sp = nullptr;                       // Cache of the current stack pointer
  std::byte *m_lo = nullptr;                       // Cache of the low address
}
