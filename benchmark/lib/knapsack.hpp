...
inline constexpr int knapsack_test = 16;
inline constexpr int knapsack_base = 28;
...
inline auto knapsack_make(int n, std::uint64_t seed = 0xCAFEBABE) -> knapsack_problem {
  std::mt19937_64 rng{seed};
  std::uniform_int_distribution<int> dw(1, 100);
  std::uniform_int_distribution<int> dv(1, 100);

  std::vector<knapsack_item> items(static_cast<std::size_t>(n));
  int total = 0;
  for (auto &it : items) {
    it.weight = dw(rng);
    it.value = dv(rng);
...
inline auto knapsack_dp_optimum(knapsack_problem const &p) -> int {
  ...
  for (auto const &it : p.items) {
    for (int c = p.capacity; c >= it.weight; --c) {
      int cand = dp[static_cast<std::size_t>(c - it.weight)] + it.value;
