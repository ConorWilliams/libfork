...
constexpr std::array<std::size_t, 11> k_sizes{1, 2, 3, 4, 5, 6, 9, 8, 97, 1024, 4028};
...
//
//       auto recv = lf::schedule(pool, lf::for_each, std::span<int>(v), std::ptrdiff_t{1}, [](int &x)
//       noexcept {
//         x = -x;
//       });
//       std::move(recv).get();
...
//
//   SECTION("iterator pair, n>1") {
//     auto recv = lf::schedule(pool, lf::for_each, v.begin(), v.end(), std::ptrdiff_t{8}, [&](int &)
//     noexcept
//     {
//       calls.fetch_add(1, std::memory_order_relaxed);
//     });
