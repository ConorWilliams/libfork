...
#include "libfork/concepts.hpp"       // for returnable
#include "libfork/detail/macro.hpp"   // for LF_ASSERT, unreachable, LF_COMPILER_EXCEPTIONS
#include "libfork/detail/utility.hpp" // for empty_t, else_empty_t, immovable, non_void
 ...
 */
template <returnable T>
using eventually_value_t = typename eventually_value<T>::type;
...
template <returnable T, bool Exception>
  requires non_void<T> || Exception
class basic_eventually : detail::immovable<basic_eventually<T, Exception>> {
  ...
  static constexpr bool is_val_value = !is_void && !is_ref_value;
  ...
  union {
    detail::eventually_value_t<T> m_value;                           //
    detail::empty_t<> m_empty;                                       //
    detail::else_empty_t<Exception, std::exception_ptr> m_exception; //
  };

  // This empty type needs to be different from the empty types in the union
  // to allow [[no_unique_address]] to work, hence the 1.
  [[no_unique_address]] detail::else_empty_t<!implicit_state, state, 1> m_flag;
