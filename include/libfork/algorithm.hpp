...
// inline constexpr auto wrap = lf::async([]<class F, class... Args>(auto self, F &&f, Args &&...args) -> lf::task<std::invoke_result_t<F, Args...>> {
//   co_return std::invoke(std::forward<decltype(f)>(f), std::forward<decltype(args)>(args)...);
// });

// }

// inline constexpr auto for_each = lf::async([](auto for_each, auto beg, auto end, auto const &fun, std::ptrdiff_t const &chunk_size) -> lf::task<void> {
//   if
// });
