...
struct stack_span {
   ...
   */
  [[nodiscard]] auto capacity() const noexcept -> std::size_t { return checked_cast<std::size_t>(hi - lo); }
  /**
   * @brief Unused space on the current stacklet's stack.
   */
  [[nodiscard]] auto unused() const noexcept -> std::size_t { return checked_cast<std::size_t>(hi - sp); }
  /**
...
class alignas(detail::k_new_align) stacklet : public detail::stack_span {
 ...
 private:
   ...
   * ahead of the top stacklet then it will be freed before being replaced with 'new_next'.
   */
  void set_next(stacklet *new_next) noexcept;

  /**
   * @brief Allocate a new stacklet with a stack of size of at least `size` and attach it
   * to the given stacklet chain.
   *
   * Requires that `prev` must be the top stacklet in a chain or `nullptr`. This will round
   * size up to a multiple of the alignment/page size.
   *
 ...
 * stack is empty. The top stacklet may have zero or one cached stacklets "ahead" of it. A stacklet may not
 * be destructed until it is null or empty.
 */
class stack_ptr : std::unique_ptr<stacklet, detail::stacklet_deleter> {
  ...
  /**
   * @brief Allocate a stacklet large enough to hold `size` bytes and attach it to the current stack.
   */
