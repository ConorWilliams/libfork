 ...
 *
 * By default this is a no-op. Defining ``LF_DEFAULT_LOGGING`` will enable a
 * default logging implementation which prints to ``std::cout``. Overridable by
 * defining your own ``LF_LOG`` macro with an API like ``std::format()``.
 */
#ifndef LF_LOG
...
/**
 * @brief Expands to `#pragma unroll n` or equivalent if the compiler supports
 * it.
 */
#ifdef __clang__
