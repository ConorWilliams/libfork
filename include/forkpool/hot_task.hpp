        ...
        struct initial_awaitable {
            constexpr bool await_ready() const noexcept {
                if constexpr (requires { Scheduler::await_ready(); }) {
                    static_assert(noexcept(Scheduler::await_ready()), "<- must not throw!");
            ...
            constexpr void await_suspend(std::coroutine_handle<promise_type> handle) const noexcept {
                try {
                    ...
                    handle.promise().set_continuation(nullptr);

                    // Release block as someone may try to call .get() on hot_task
            ...
            constexpr void await_resume() const noexcept {
                // TODO : Could cancel early if _continuation == nullptr, not sure if possible without
